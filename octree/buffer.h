...
class Buffer {
 public:
  Buffer() : m_ptr(0) {}
  // Buffer( size_t count=0, RTPbuffertype type=RTP_BUFFER_TYPE_HOST,
  // PageLockedState pageLockedState=UNLOCKED )
  //   : m_ptr( 0 )
  // {
  ...
  void alloc(size_t count, RTPbuffertype type,
             PageLockedState pageLockedState = UNLOCKED) {
    if (m_ptr) free();
    ...
    if (m_count > 0) {
      if (m_type == RTP_BUFFER_TYPE_HOST) {
        ...
        if (pageLockedState)
          rtpHostBufferLock(
              m_ptr, sizeInBytes());  // for improved transfer performance
  ...
  void free() {
    if (m_ptr && m_type == RTP_BUFFER_TYPE_HOST) {
      if (m_pageLockedState) rtpHostBufferUnlock(m_ptr);
      delete[] m_ptr;
    } else {
      int oldDevice;
