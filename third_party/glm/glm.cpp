 ...
 */
static float _glmDot(float* u, float* v) {
  assert(u);
  assert(v);
 ...
 */
static void _glmCross(float* u, float* v, float* n) {
  assert(u);
  assert(v);
  assert(n);
 ...
 * v - array of 3 floats (float v[3])
 */
static bool _glmEqual(float* u, float* v, float epsilon) {
  if (_glmAbs(u[0] - v[0]) < epsilon && _glmAbs(u[1] - v[1]) < epsilon &&
 ...
 *
 */
static float* _glmWeldVectors(float* vectors, unsigned int* numvectors,
                              float epsilon) {
  float* copies;
  unsigned int copied;
  unsigned int i, j;
...
*/
static GLMgroup* _glmFindGroup(GLMmodel* model, char* name) {
  GLMgroup* group;
...
*/
static unsigned int _glmFindMaterial(GLMmodel* model, char* name) {
  unsigned int i;
 ...
 */
static int _glmReadMTL(GLMmodel* model, char* name) {
  FILE* file;
  char* dir;
  char* filename;
  char buf[2048];
  ...
  while (fscanf(file, "%s", buf) != EOF) {
    switch (buf[0]) {
      ...
      case 'r':  // reflectivity
        ...
        break;
      case 'e':  // emissive
        fscanf(file, "%f %f %f", &model->materials[nummaterials].emissive[0],
      ...
      case 'K':
        switch (buf[1]) {
          case 'd':
            fscanf(file, "%f %f %f", &model->materials[nummaterials].diffuse[0],
          ...
          case 's':
            ...
            break;
          case 'a':
            fscanf(file, "%f %f %f", &model->materials[nummaterials].ambient[0],
 ...
 */
static int _glmWriteMTL(GLMmodel* model, char* modelpath, char* mtllibname) {
  FILE* file;
  char* dir;
  char* filename;
 ...
 */
static int _glmFirstPass(GLMmodel* model, FILE* file) {
  ...
  while (fscanf(file, "%s", buf) != EOF) {
    switch (buf[0]) {
      ...
      case 'u':
        ...
        sprintf(buf, "%s_MAT_%s", current_group_base_name,
  ...
  while (group) {
    group->triangles =
        (unsigned int*)malloc(sizeof(unsigned int) * group->numtriangles);
    group->numtriangles = 0;
    group = group->next;
  }
  ...
  while (fscanf(file, "%s", buf) != EOF) {
    switch (buf[0]) {
        ...
        switch (buf[1]) {
          case '\0': /* vertex */
            if (!model->usePerVertexColors) {
              fscanf(file, "%f %f %f", &vertices[3 * numvertices + X],
                     &vertices[3 * numvertices + Y],
                     &vertices[3 * numvertices + Z]);
            } else {
              int r, g, b;
              fscanf(file, "%f %f %f %d %d %d", &vertices[3 * numvertices + X],
                     &vertices[3 * numvertices + Y],
            ...
            break;
          case 'n': /* normal */
            fscanf(file, "%f %f %f", &normals[3 * numnormals + X],
                   &normals[3 * numnormals + Y], &normals[3 * numnormals + Z]);
            numnormals++;
            break;
          case 't': /* texcoord */
            fscanf(file, "%f %f", &texcoords[2 * numtexcoords + X],
      ...
      case 'u':  // usemtl
        ...
        sprintf(buf, "%s_MAT_%s", grpname, mtlname);
...
float glmUnitize(GLMmodel* model) {
  ...
  for (i = 1; i <= model->numvertices; i++) {
    if (maxx < model->vertices[3 * i + X]) maxx = model->vertices[3 * i + X];
    if (minx > model->vertices[3 * i + X]) minx = model->vertices[3 * i + X];

    if (maxy < model->vertices[3 * i + Y]) maxy = model->vertices[3 * i + Y];
    if (miny > model->vertices[3 * i + Y]) miny = model->vertices[3 * i + Y];

    if (maxz < model->vertices[3 * i + Z]) maxz = model->vertices[3 * i + Z];
    if (minz > model->vertices[3 * i + Z]) minz = model->vertices[3 * i + Z];
  }
 ...
 */
void glmBoundingBox(GLMmodel* model, float* minpos, float* maxpos) {
  unsigned int i;
  float maxx, minx, maxy, miny, maxz, minz;
...
void glmFacetNormals(GLMmodel* model) {
  ...
  model->facetnorms =
      (float*)malloc(sizeof(float) * 3 * (model->numfacetnorms + 1));
...
void glmLinearTexture(GLMmodel* model) {
  ...
  model->texcoords =
      (float*)malloc(sizeof(float) * 2 * (model->numtexcoords + 1));

  glmDimensions(model, dimensions);
  scalefactor = 2.0f / _glmAbs(_glmMax(_glmMax(dimensions[0], dimensions[1]),
                                       dimensions[2]));
...
GLMmodel* glmReadOBJ(const char* filename) {
  ...
  /* allocate memory */
  model->vertices =
      (float*)malloc(sizeof(float) * 3 * (model->numvertices + 1));
  model->vertexColors = (unsigned char*)malloc(sizeof(unsigned char) * 3 *
                                               (model->numvertices + 1));
  model->triangles =
      (GLMtriangle*)malloc(sizeof(GLMtriangle) * model->numtriangles);
  if (model->numnormals) {
    model->normals =
        (float*)malloc(sizeof(float) * 3 * (model->numnormals + 1));
  }
  if (model->numtexcoords) {
    model->texcoords =
        (float*)malloc(sizeof(float) * 2 * (model->numtexcoords + 1));
 ...
 */
int glmWriteOBJ(GLMmodel* model, char* filename, unsigned int mode) {
  unsigned int i;
  FILE* file;
  GLMgroup* group;
