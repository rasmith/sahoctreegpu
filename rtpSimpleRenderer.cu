...
__global__ void createRaysOrthoKernel(float4* rays, int width, int height,
                                      float x0, float y0, float z, float dx,
                                      float dy) {
...
RTPSimpleRenderer::RTPSimpleRenderer(const ConfigLoader& c)
    : Renderer(c) {
  // Create buffer for ray input
  rayBuffer.alloc(0, c.bufferType, LOCKED);
  createRaysOrtho();
  CHK_PRIME(rtpBufferDescCreate(
      context, Ray::format, /*RTP_BUFFER_FORMAT_RAY_ORIGIN_TMIN_DIRECTION_TMAX*/
      rayBuffer.type(), rayBuffer.ptr(), &rayDesc));
  CHK_PRIME(rtpBufferDescSetRange(rayDesc, 0, rayBuffer.count()));

  // Create buffer for returned hit descriptions
  hitBuffer.alloc(rayBuffer.count(), c.bufferType, LOCKED);
  CHK_PRIME(rtpBufferDescCreate(
      context, Hit::format, /*RTP_BUFFER_FORMAT_HIT_T_TRIID_U_V*/
      hitBuffer.type(), hitBuffer.ptr(), &hitDesc));
  CHK_PRIME(rtpBufferDescSetRange(hitDesc, 0, hitBuffer.count()));
}

void RTPSimpleRenderer::createRaysOrtho(float margin, int yOffset,
                                        int yStride) {
...
void RTPSimpleRenderer::createRaysOrthoOnDevice(float x0, float y0, float z,
                                                float dx, float dy, int yOffset,
                                                int yStride) {
  int rows = idivCeil((image.height - yOffset), yStride);
  ...
  createRaysOrthoKernel << <gridSize, blockSize>>>
      ((float4*)rayBuffer.ptr(), image.width, rows, x0, y0 + dy * yOffset, z,
       dx, dy * yStride);
}
